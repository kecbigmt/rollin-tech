...
#### 解答
```
# 6.mathモジュールをimportして平方根を計算しよう
import math

# 5.関数を定義しよう
def dist(point1, point2):
  # 1.変数に値を保存しよう / 4.タプルにxy座標を保存しよう
  x1 = point1[0]
  y1 = point1[1]
  x2 = point2[0]
  y2 = point2[1]

  # 2.四則演算で「2点間の距離の公式」の一部を表現しよう
  r = (x1 - x2)**2 + (y1 - y2)**2
...
### 4. データ構造
データの保持の仕方のこと。ここで上げたものの他にも[集合型（set）](https://docs.python.jp/3/tutorial/datastructures.html#sets)というのもあるが、ここでは扱わない
...
#### パッケージ
有志が作ったパッケージを利用することでもっといろいろなことができるようになる
※詳細は次回
